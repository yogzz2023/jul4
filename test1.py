...
filtered_values_csv = pd.read_csv('ttk_52_test.csv')
filtered_time = filtered_values_csv[['MT']].values
measurement_groups = form_measurement_groups(read_measurements_from_csv('ttk_52_test.csv'))
...
for group_index, group in enumerate(measurement_groups):
    for i, (x, y, z, mt) in enumerate(group):
        ...
        if i == 0 and group_index == 0:  # First measurement of the first group
            kalman_filter.initialize_filter_state(x, y, z, 0, 0, 0, mt)
            targets.append(kalman_filter.Sp)
            kalman_filter.first_step_flag = True
            kalman_filter.second_step_flag = False
        elif i == 1 and group_index == 0:  # Second measurement of the first group
            ...
            targets.append(kalman_filter.Sp)
            kalman_filter.second_step_flag = False
        elif i >= 2:  # Starting from the third measurement
